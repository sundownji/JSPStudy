<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
/* 해당 페이지에서는 가장 먼저 팝업창 오픈에 관련된 쿠키가 있는지 
먼저 확인한 후 팝업창을 처리해야한다. */
//팝업창 오픈 여부 판단을 위한 변수
String popupMode = "on"; 
...
$(function() {
	...
	$('#closeBtn').click(function(){
		//레이어 팝업창을 숨김처리한다. (display(속성을)=none(으로 만든다.))
		$('#popup').hide();
		...
		$.ajax({
			...
			success : function(resData){
				/*콜백데이터에 지시어 때문에 남아 있는 공백으로 인해
				불필요한 새로고침이 일어날 수 있다.
				따라서 공백을 제거해야 정상적으로 동작하게된다.*/
				if(resData){
					console.log('있다');
				}
				else{
...
<%
	...
	/* 만약 변수값이 on이 아니라면 아래 팝업창을 위한 html 코드는 물리적으로 삽입되지 않는다.
