...
<body>
	...
	<!--  "{}" 중괄호랑 " 사이에 공백이 있으면 오류가 발생함 -->
	<!-- if(number%2==0) 자바식과 동일한 조건의 if문으로 해당 조건의 결과가 result에 저장된다. 
	...
	<!-- JSTL의 if태그는 else구문이 별도로 없기 때문에 if문과의 반대조건을
		만들어서 두개의 if 태그를 사용한다.  -->
	<!-- else구문이 없기 때문에 if문의 반대구문을 만들어 사용한다  -->
	<h4>문자열 비교와 else 구문 흉내내기</h4>
	<!-- Java에서는 비교연산자 ==와 equals()는 다르지만, 
		EL에서는 eq를 통해 값에 대한 비교와 문자열에 대한 비교 모두 할 수 있다.
		여기서 사용한 eq는 Java의 compareTo()와 같이 사전순으로 문자열을 	비교한다.   -->
	<!-- 첫번째 if문은 false의 결과가 나온다. string="JSP"임 -->
	...
	<!-- test에는 앞뒤에 공백이 하나라도 들어가면 무조건 false를 반환하게된다.
		따라서 주의해아한다.  -->
	...
	<c:if test="${ not empty param.user }">
		...
		<c:if test = "${ param.user eq 'tjoeun' &&  param.pass eq '1234' }" var="loginresult">
			${param.user }님 안녕하세요.
		</c:if>
