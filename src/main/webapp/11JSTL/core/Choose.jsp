...
<body>
	<!--
	choose태그
		:하위에 when, otherwise 태그를 가지고있다.
		여러조건이 있는 경우 사용하기에 적합하다.
	  -->
	<!--변수 선언  -->
	...
	<c:choose>
		...
		<c:otherwise>
		<!-- else와 동일한 역할을 한다.
			위 조건에 만족하지 않을 때 실행된다.  -->
			${ number }는 홀수입니다.
		</c:otherwise>
	</c:choose>
	
	<h4>국,영,수 점수를 입력하면 평균을 내어 학점 출력</h4>
	<!--form 속성 아무것도 없으면 get방식에 현재페이지로 폼값이 전송된다.  -->
	<form>
	...
	<c:if test="${not (empty param.kor or empty param.eng or empty param.math) }">
		<!-- 3과목의 점수를 더한 후 평균값을 계산한다. 이 값은 avg에 저장된다.
			EL은 자동으로 형변환 되므로 경우에 따라 실수의 결과가 나올수도 있다.   -->
		<c:set var="avg" value="${ (param.kor + param.eng + param.math) / 3 }" />
		평균 점수는 ${avg }으로
		
		<!-- 평균점수를 통해 조건을 걸어 학점을 출력한다. 
			when태그는 if 혹은 else if와 동일하고
			otherwise는 else와 동일하다.  -->
		<c:choose>
