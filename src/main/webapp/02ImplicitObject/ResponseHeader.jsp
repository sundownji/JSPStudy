...
//응답 헤더에 추가할 값 준비
/*
get방식으로 전송된 폼값을 날짜 형식을 통해 타임스템프로 변경한다.
getTime(): 날짜를 1970년부터 지금까지의 시간을 초단위로 변경하여 반환해준다.
*/

//문자열 형식을 날짜 형식으로 변형하기 위해 포맷을 설정한다.
/*
응답헤더에 날짜를 지정하는 경우 대한민국은 세계표준시(그리니치천문대)에서 +09시 빠르므로
그만큼을 더해줘야 정상적인 날짜가 출력된다. 만약 09시 이전의 시간으로 세팅되면 하루전의 날짜가 출력된다.
*/
//응답헤더에 시간 형식은 작성하지 않았기 때문에 표준시간을 기준으로 -9시간된 날짜와 시간이 출력됨
...
/* <form>태그를 통해 서버로 전송되는 값은 모두 String 타입이므로 숫자나 날짜형식의
연산이 필요한 경우에는 변환 후 사용해야한다.*/

//응답 헤더에 값 추가
/*
addDateHeader(헤더명, long타입의 타임스템프)
	: 응답헤더에 날짜형식을 추가하는 경우 long타입의 타임스템프로 변환 후 추가하면된다.
addIntHeader(): 숫자형식의 응답헤더를 추가한다.
addHeader(): 문자형식의 응답헤더를 추가한다.
*/
//날짜형식 추가
response.addDateHeader("myBirthday", add_date);
//정수형식 추가(동일한 헤더명으로 2개의 값을 추가함)
